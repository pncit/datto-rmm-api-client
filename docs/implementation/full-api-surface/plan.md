...
- `tests/generated/widen-enums.test.ts`: unit-test the codemod against a fixture string containing an enum in a component-schema type (e.g. `Device.deviceClass`), an enum in a `*Body` type, and an enum in a `*Params` type; assert **only** the component-schema (response) enum gains `| (string & {})` while the `*Body`/`*Params` enums stay closed (guards the over-widen risk that would silently loosen the request-side type contract), and that a second pass is a no-op. Add two cases for the **shared-schema guard**: (i) a spec fragment where a request body **directly** `$ref`s a component schema also used as a response makes the guard **throw**; and (ii) a fragment where a request body reaches the shared component schema only **transitively** (via a nested property `$ref`) still makes the guard **throw** — proving the guard resolves refs recursively and the load-bearing assumption is verified at every depth, not just at the top level.
...
// Recursive completeness guard: every enum field's CONTAINING top-level property must be in WIDENED_FIELDS.
for (const [entity, fields] of Object.entries(WIDENED_FIELDS)) {
  // enumFieldPaths is imported from src/validation/schema-leniency.ts (the single _zod.def-isolated site)
  for (const path of enumFieldPaths(entity)) {   // recurses nested objects → e.g. 'antivirus.antivirusStatus'
    expect(fields).toContain(topLevelOf(path));  // containing top-level field is grafted widened
  }
}
```
