...
4. **Write the response-enum-widening codemod** `scripts/widen-response-enums.mjs`: after Orval runs, rewrite every **response** enum field so its emitted TypeScript type is the open form `EnumUnion | (string & {})` (R5), deterministically across `src/generated/`. This idiom has no JSON-Schema representation, so it must be a post-generate script, not an Orval hook. Keep the transform idempotent (running twice is a no-op) so reproducibility holds.
   ...
   - **Verify the load-bearing assumption (do not just trust it) — and verify it *transitively*, not just at the top level:** the whole rule rests on "no request body reaches a component schema that is also reachable from a response." The codemod widens **every** exported component-schema type in `src/generated/types/**` that lacks a request-side suffix — including **nested** object types a `*Body` references (`{ status: { $ref: '#/components/schemas/SomeEnumHolder' } }`). So a shallow guard that inspects only the **top-level** `requestBody` `$ref` is insufficient: it would pass even when a write body reaches a **nested** component schema that is also reached from a response, silently over-widening that shared nested enum on the compile-time **request** type (a caller could pass a novel string that type-checks but is then rejected at runtime by the strict `validateRequest`/write-body override in Phase 6). Add a **transitive** guard — a scan of the patched spec (`spec/openapi.patched.json`) run as the first thing the codemod does (or a standalone assertion in `widen-enums.test.ts`) that, for every operation's `requestBody.content['application/json'].schema`, **resolves `$ref`s recursively** (following nested `$ref`s through `properties`/`items`/`allOf`/`oneOf`/`anyOf`/`additionalProperties`, with a visited-set to terminate cycles) to collect the **full transitive set** of `#/components/schemas/*` names the request body reaches; likewise collect the transitive set reachable from every response schema; then **throw** if those two sets intersect **on a component that actually carries an enum**. **Filter the intersection to enum-bearing shares — do not throw on a benign shared non-enum component.** Widening is a strict no-op on an enum-free schema, so a shared address/pagination/metadata sub-object commonly referenced by both a write body and a response cannot be over-widened; because recursing through `properties`/`items`/`allOf`/… now dives into exactly those deep, widely-shared building blocks, throwing on every name-set overlap would fail `npm run generate` for a harmless share. So the guard throws only for a shared component **whose own schema (or a nested schema within it) declares an `enum`** — precisely the case where over-widening a request-side enum is possible; an enum-free shared component is silently allowed. If the (enum-bearing) guard ever fires, the suffix heuristic could silently widen a shared (direct **or** nested) request-body enum, so it fails the build and forces an explicit fix rather than a silent request-type loosening.
     - **Fail-loud message contract (self-locating, mirrors patch-spec l.153/l.187):** the `throw new Error(...)` **names the offending intersecting `#/components/schemas/*` component(s)** and, for each, the request-body and response operation(s) that reach it (e.g. `widen-enums: request/response share enum-bearing schema(s): SomeEnumHolder (reached by requestBody of POST /device/{uid}/foo and response of GET /device/{uid}) — split the shared schema or add a request-side suffix`). Without named anchors the maintainer gets an opaque build break with no pointer to the schema to split; naming them makes the fix obvious.
   - Files: `scripts/widen-response-enums.mjs`
   - Notes: Runtime enum *degradation* is a separate mechanism handled in Phase 4's `parseLenient`; this codemod only widens the compile-time type. Phase 9 asserts the two stay aligned on the same field set, at every nesting depth.
...
- `tests/generated/widen-enums.test.ts`: unit-test the codemod against a fixture string containing an enum in a component-schema type (e.g. `Device.deviceClass`), an enum in a `*Body` type, and an enum in a `*Params` type; assert **only** the component-schema (response) enum gains `| (string & {})` while the `*Body`/`*Params` enums stay closed (guards the over-widen risk that would silently loosen the request-side type contract), and that a second pass is a no-op. Add three cases for the **shared-schema guard**, each with the shared component carrying an `enum` so the throw is meaningful: (i) a spec fragment where a request body **directly** `$ref`s an **enum-bearing** component schema also used as a response makes the guard **throw**; (ii) a fragment where a request body reaches the shared **enum-bearing** component schema only **transitively** (via a nested property `$ref`) still makes the guard **throw** — proving the guard resolves refs recursively and the load-bearing assumption is verified at every depth, not just at the top level; and (iii) a fragment where a request body and a response share a component that is **enum-free** (a benign address/metadata sub-object) does **not** throw — proving the guard fires only when over-widening is actually possible and does not spuriously break `npm run generate` on a harmless share. Also assert the thrown `Error` message **names the offending intersecting component(s)** (the self-locating fail-loud contract), mirroring the `patch-spec` drift-message test.
...
### Steps
1. **Port** `fuze-api/src/validation/schema-leniency.ts` → `src/validation/schema-leniency.ts` verbatim (keeps all `_zod.def` access isolated here, per the risk-mitigation in the design). This gives `parseLenient(schema, data, logger?, context?)`, `addCatchallRecursive`, `detectUnknownProperties`. **Also export from this module a small `enumFieldPaths(schema): string[]` helper** that reuses the same recursive walk to return the dotted path of every `enum` node at every depth (e.g. `['deviceClass', 'antivirus.antivirusStatus', 'patchManagement.patchStatus']`); Phase 9's recursive `WIDENED_FIELDS` completeness guard imports it. Keeping it here confines **all** `_zod.def` introspection to this one file (per the design's isolation mitigation) instead of adding a second parallel site in test code.
...
3. **Schema-override module** `src/schema-overrides/index.ts` (+ split files as needed): named to match the design (`src/schema-overrides.ts`, R8); this repo splits it into a directory for readability. It overrides the **generated zod schemas** (not the OpenAPI spec) — do not confuse it with Phase 2's `scripts/patch-spec.mjs`, which corrects the spec *before* generation; this module reconciles the *already-generated* schemas *after* generation. It lives outside `src/generated/`, imports generated zod schemas, and exports the reconciled forms resources use:
   ...
   - **Reconciled entity types are the single source of truth (R4/R5 alignment):** for every entity this module reconciles (`Device`, `Alert`, and any other override-touched schema), the public TypeScript type is derived **primarily from the override schema's `z.infer`** (which carries the reconciled `udfSchema` record and the open `@class` `alertContext`) — *not* the raw generated type, whose pre-reconciliation shape (literal `udf1…udf300` props, generated `alertContext`) does **not** match what the resources validate and return.
     - **The open-enum widening must be grafted on explicitly, at every depth — `z.infer` alone does not carry it.** The R5 `(string & {})` open-enum widening is a **TS-type-only** transform the Phase 2 codemod applies to `src/generated/types/**` (at every nesting depth); it has no runtime/zod representation, so `z.infer` of a composed override schema does **not** inherit it. Composing the generated zod enum yields either a **closed** union (a novel value fails to type-check — reintroducing the exact "compile-time claims an exhaustiveness the runtime relaxes" hazard R5 exists to kill) or, via `.or(z.string())`, a **collapsed plain `string`** (losing the literal members). Neither is the R5 shape. Therefore define each reconciled entity type as an **intersection that takes each enum-bearing top-level field's whole subtree from the codemod-widened generated type and everything else from `z.infer<override>`** — and **drive the `Omit`/`Pick` key set from one `as const` constant per entity, never hand-repeated literals**, so the graft and the documented field list cannot desync (the same single-source discipline the plan applies to `WriteOpKey`/`DEFAULT_RETRY`). **The constant lists top-level property names, and each must name the top-level field whose subtree contains an open enum *anywhere* — including an enum nested inside a sub-object.** `Device` really carries **nested** enums (`antivirus.antivirusStatus`, `patchManagement.patchStatus` — design l.188's "25 enums… `antivirusStatus`, `patchStatus`") in addition to the top-level `deviceClass`; a `Pick` key can only address a **top-level** property, so the constant lists the **containing** top-level fields: `DEVICE_WIDENED_FIELDS = ['deviceClass', 'antivirus', 'patchManagement'] as const` (and `ALERT_WIDENED_FIELDS`, etc. — one per override-touched entity, listing every top-level field whose subtree carries any open enum). **Pair each constant with its override schema in one per-entity registry `OVERRIDE_ENTITIES` (in `schema-overrides/types.ts`) — the single place Phase 6's graft and Phase 9's guard both read:** `export const OVERRIDE_ENTITIES = [{ name: 'Device', schema: deviceResponseSchema, widenedFields: DEVICE_WIDENED_FIELDS }, { name: 'Alert', schema: alertResponseSchema, widenedFields: ALERT_WIDENED_FIELDS }, …] as const`. Because the registry carries the **override schema object** (not just the entity-name string) alongside its `widenedFields`, Phase 9's completeness guard can feed each `schema` straight to `enumFieldPaths(schema: z.ZodType)` (which introspects `_zod.def` and needs a schema, never a name) and check the paths against that same entry's `widenedFields` — with no entity-name→schema lookup and, critically, **no reverse dependency** from `schema-leniency.ts` back into the override registry (`enumFieldPaths` stays typed to accept a `z.ZodType`, so the Phase 4 validation module never imports Phase 6). **The name is deliberate: each entry is the *containing top-level property* whose subtree is grafted widened — not a literal enum-typed field.** `antivirus`/`patchManagement` are sub-objects, not enums; the constant is named `*_WIDENED_FIELDS` (not `*_ENUM_FIELDS`) precisely so an implementor working from the name alone populates it with the **top-level `Pick` key** whose subtree holds an open enum at any depth, and is not misled into listing the nested enum names (`antivirusStatus`/`patchStatus`, which are not top-level `Pick` keys and won't compile) or just `['deviceClass']` (omitting the containing fields and reviving the nested-enum-typed-closed bug this graft exists to prevent). Write the type as `export type Device = Omit<z.infer<typeof deviceResponseSchema>, typeof DEVICE_WIDENED_FIELDS[number]> & Pick<GeneratedDevice, typeof DEVICE_WIDENED_FIELDS[number]>` where `GeneratedDevice` is imported from `src/generated/types`; because `Pick<GeneratedDevice, 'antivirus'>` re-adds the **whole** `antivirus` sub-object from the fully-widened generated type, the nested `antivirusStatus`/`patchStatus` enums are widened along with the top-level `deviceClass`. Because the `Omit`/`Pick` keys are `typeof DEVICE_WIDENED_FIELDS[number]`, the constant is the **single source** for which top-level subtrees are taken widened — adding a field updates the graft automatically, with no hand-written key literals to fall out of sync. **Constraint (must hold for this top-level graft to be sound):** an entity's genuinely-reconciled fields (`udf`, `alertContext`) must **not** overlap an enum-bearing subtree and must themselves contain **no** enums — which holds here (`udf` is a scalar record, `alertContext` an open `@class` object, neither carrying an enum). If a future reconciliation ever touched a field that *also* contained a nested enum, the top-level graft could not widen it and the type would need to compose at that field's sub-object level; Phase 9's recursive guard (below) catches that case by failing rather than shipping a closed nested enum. This makes the exported compile-time type carry the reconciled fields **and** the open-enum widening at every depth, matching what `parseLenient`'s runtime enum degradation (Phase 4) accepts. The per-entity `WIDENED_FIELDS` constants are documented in `types.ts`; Phase 9 both drives a **truly-novel-value** compile assertion over **every** enum field — **top-level and nested** — of **every** override-touched entity (e.g. `Device['deviceClass']`, `Device['antivirus']['antivirusStatus']`, `Device['patchManagement']['patchStatus']`, and each `Alert` enum field) **and** runs a completeness guard that **recursively** enumerates every enum-typed field present on each override-touched entity and asserts its **containing top-level property** is listed in that entity's `WIDENED_FIELDS` constant (so a nested or top-level enum whose containing field is omitted — which would type **closed** at compile time while `parseLenient` widens it at runtime — fails the gate rather than silently typing closed) — this is what turns the "graft and runtime widening cover the same field set at every depth" property into a verified gate rather than an unchecked claim.
   ...
   - Files: `src/schema-overrides/index.ts` (+ e.g. `device-overrides.ts`, `alert-overrides.ts`, `pagination.ts`, `write-bodies.ts`, `types.ts` for the reconciled entity types — the `z.infer` base intersected with the codemod-widened generated **enum-bearing top-level subtrees** per each entity's `WIDENED_FIELDS`)
...
### Opinionated Implementation Notes (Examples)
```ts
import type { Device } from '@/schema-overrides/types'; // reconciled type (z.infer base + widened generated enum-bearing subtrees grafted, incl. nested), NOT the raw generated Device
...
npm test
npm run build
! git grep -n "Result<" -- src/ | grep -v "ZodSafeParseResult"                        # Result contract fully removed (zod's ZodSafeParseResult is not our retired Result<T>)
! git grep -n "validationMode" -- src/ | grep -vE '^[^:]+:[0-9]+:[[:space:]]*(\*|//|/\*)'  # three-mode config fully removed (doc-comment lines excluded)
...
3. **Fixture-validation tests** `tests/integration/fixtures.test.ts`: parse each fixture through its reconciled schema via the resource/`parseLenient` path and assert:
   ...
   - The `rmmnetworkdevice`/novel-enum fixture both **type-checks** against the reconciled, override-derived response type **and** survives `parseLenient` without being dropped — asserting the build-time and runtime enum widening cover the same field set at every depth (Success Criteria, R5). The compile-time assertion must use a **truly novel** value (`'quantumdevice'`, not the already-declared `'rmmnetworkdevice'` member), and it must be driven over **every** enum field — **top-level and nested** — of **every** override-touched entity (Phase 6 Step 3): `Device['deviceClass']`, the **nested** `Device['antivirus']['antivirusStatus']` and `Device['patchManagement']['patchStatus']`, and each `Alert` enum field — so a missing graft on any entity's open-enum field at any depth (a nested `Device` enum, or an `Alert` field) fails here rather than passing on the one top-level field that happens to be covered.
   - **Recursive completeness guard for `WIDENED_FIELDS`:** a test that iterates the per-entity `OVERRIDE_ENTITIES` registry (Phase 6 Step 3 — each entry is `{ name, schema, widenedFields }`) and, for each entry, **recursively** enumerates every enum-typed field present (top-level *and* nested, derived from the entry's override schema) and asserts each field's **containing top-level property** is listed in that entry's `widenedFields` constant. **The recursive enumerator `enumFieldPaths(schema: z.ZodType)` is an exported helper of `src/validation/schema-leniency.ts`, built on that module's existing `addCatchallRecursive`/`detectUnknownProperties` walk (which already visits every `enum` node at every depth), and the guard imports it from there — it does NOT re-introspect zod internals in test code, and it takes the entry's `schema` object (never a name string, which has no `_zod.def`), so `schema-leniency.ts` never imports the Phase 6 override registry (no reverse dependency / layering inversion).** TS types are not enumerable at runtime, so enumerating every enum field at arbitrary depth requires structural `_zod.def` access on the override/generated zod schema; the plan deliberately confines all `_zod.def` access to `schema-leniency.ts` (Step 4/Phase 4 Step 1) so a Zod major bump breaks exactly one file. Homing `enumFieldPaths` there (rather than as a second, parallel introspection site in the Phase 9 test) preserves that single-isolation-boundary mitigation **and** guarantees the guard cannot desync from the very walker that widens enums at runtime — both read enum nodes through the same code. This binds the hand-maintained constant to the actual enum fields present at every depth, so a spec-refresh enum — a new top-level one **or** one nested inside a sub-object (`antivirus.*`, `patchManagement.*`) — whose containing top-level field is omitted from the constant (which would type **closed** at compile time while `parseLenient` widens it at runtime, reviving the exact R5 hazard for that field) fails the gate rather than shipping silently.
...
// TRULY NOVEL values (not the declared members): each only type-checks if the reconciled Device
// type carries the codemod-widened `(string & {})` graft at that depth (Phase 6 Step 3 —
// Pick<GeneratedDevice, 'antivirus'|'patchManagement'|'deviceClass'> grafts whole widened subtrees).
const dc: Device['deviceClass'] = 'quantumdevice';                    // top-level enum
const av: Device['antivirus']['antivirusStatus'] = 'QuantumAV';      // NESTED enum
const ps: Device['patchManagement']['patchStatus'] = 'QuantumPatch'; // NESTED enum
expect(() => validateResponse(wire)).not.toThrow();
expect(validateResponse(wire).deviceClass).toBe('rmmnetworkdevice'); // not dropped

// The same truly-novel assertion is written for EVERY enum field — top-level AND nested — of
// EVERY override-touched entity (e.g. an Alert enum field), so a missing graft at any depth fails.
// Recursive completeness guard: every enum field's CONTAINING top-level property must be in widenedFields.
// Iterate the OVERRIDE_ENTITIES registry (Phase 6 Step 3) — each entry pairs a SCHEMA with its widenedFields,
// so enumFieldPaths gets a z.ZodType (has _zod.def), never a name string, and schema-leniency.ts never
// imports the override registry (no reverse dependency).
for (const { schema, widenedFields } of OVERRIDE_ENTITIES) {
  // enumFieldPaths is imported from src/validation/schema-leniency.ts (the single _zod.def-isolated site)
  for (const path of enumFieldPaths(schema)) {   // recurses nested objects → e.g. 'antivirus.antivirusStatus'
    expect(widenedFields).toContain(topLevelOf(path)); // containing top-level field is grafted widened
  }
}
```
