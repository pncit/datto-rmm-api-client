...
`updateDeviceUdfs` — against a Datto RMM v2 surface of **53 paths / 57 operations** across ten
...
Hand-transcribing ~113 schemas and 57 operations to close the gap would multiply this class of
...
|----|-------------|------|--------|
| R1 | The client exposes **every** Datto RMM API v2 operation — all 53 paths / 57 operations across the ten namespaces `account`, `sites`, `devices`, `alerts`, `jobs`, `filters`, `audit`, `users`, `activityLogs`, `system` — covering both reads and writes. | Functional | Scope decision (full read + write) |
...
| R3 | Paginated collections transparently walk `pageDetails.nextPageUrl` and return the **full** result set. The `pageDetails` envelope is validated by a dedicated **override** schema that models `nextPageUrl`/`prevPageUrl` as **nullable** strings and `count`/`totalCount` as present integers, and is applied **strictly on structure**: a missing or malformed `pageDetails` **throws** `DattoValidationError` rather than silently truncating the walk, while a `null` `nextPageUrl` is the normal end-of-walk terminal, not an error. Response leniency applies to the named-array items, not the walk cursor. | Functional | Existing pagination behavior, generalized |
...
| R5 | **Response** validation is **lenient**: unknown keys are stripped and logged, all fields are tolerated as nullable/optional (the spec carries no reliable presence/nullability information), and **enum-typed fields degrade to passthrough** — an unobserved value is logged but widened to `string` rather than failing the item. The emitted **TypeScript response type** for each such field is widened to match the runtime contract (`EnumUnion \| (string & {})`), so the compile-time type does not claim an exhaustiveness the runtime deliberately relaxes; callers must handle an unknown value. | Non-functional | Reality findings |
...
| R18 | A comprehensive public **README** documents install, auth setup, per-namespace usage — including a **namespace→endpoint map** that makes the `account.devices()` (list) vs `devices.get(uid)` (single/mutate) split explicit — error handling, logger injection, validation leniency, and rate-limit config. | Functional | Stakeholder request |
...
- **Spec-patch step** (R8, R15) — a committed script transforms `spec/openapi.json` into
  ...
  object carrying `@class`. The patched spec is a **transient** regenerated artifact (the *script* is
  committed, its output is not); its role in the byte-for-byte reproducibility gate is defined in R15.
- **Schema-override module** (`src/schema-overrides.ts`, R8) — where a per-field patch is clearer
  ...
  required fields of the small write set (one place, hand-verified against the endpoint docs), and the
  **`pageDetails` cursor override** (see R3) that the strict `paginate` walk validates against. Imports generated schemas;
  is imported by resources. Survives regeneration because it lives outside `src/generated/`.
- **Lenient response parsing** (`src/validation/schema-leniency.ts`, R5, R7) — `parseLenient`
  ported from `fuze-api`, extended so **response** validation also tolerates null/absent values on any
  field (the spec cannot be trusted for presence) and **widens every enum-typed field to passthrough**
  (each generated enum is unioned with `string` on the response side, logging the unseen value rather
  than failing the item). Unknown keys are stripped and logged; array items validate independently and
  drop-and-log on failure. Enum widening is deliberate: without it, per-item drop (R7) would silently
  discard any record carrying a future server enum value — re-creating the exact `rmmnetworkdevice`
  silent-data-loss failure the Problem Statement condemns. Enums stay strict on **request** bodies,
  where the client controls the value.

  The emitted **response type** is widened to match (`EnumUnion | (string & {})`, R5), so the
  compile-time contract does not claim an exhaustiveness the runtime relaxes. This "open-enum" idiom is
  TypeScript-only with no JSON-Schema representation (a spec enum widened with a string collapses to
  plain `string`, losing the literal members), so it cannot come from an Orval spec-level hook; it is
  produced by a **committed post-generate codemod** — the second step of `npm run generate`, mirroring
  `fuze-api`'s `"generate": "orval && node scripts/…"` — that rewrites every response enum field to the
  widened form deterministically across the generated output (a script, not a hand-edit), staying
  reproducible under R15.
- **Leniency diagnostics volume & levels** (R5, R7, R13) — diagnostics are **aggregated per
  page/call**, not emitted per item: stripped-unknown-key and enum-widening events are deduped by
  `(context, field, value)` and summarized (e.g. one line `widened deviceClass=rmmnetworkdevice on
  3/848 devices`), so a fully-walked collection (848 devices, 1500 alerts) cannot produce thousands of
  lines or run the masker in a per-row hot path. Benign events (unknown-key strip, enum widening) log
  at **`debug`**; a per-item **drop** (R7 — actual data loss) logs at **`warn`**, keeping genuine
  signal separable from routine leniency noise.
- **`BaseResource`** (`src/client/resources/base-resource.ts`, R2, R6, R7) — validated
  ...
  and each page's cursor with the strict R3 `pageDetails` override. Leniency governs the item payloads,
  never the walk-control cursor.
...
- **UDF log masking** (R20) — enforced at the **single logger boundary**: the injected `DattoLogger`
  is wrapped in a masking decorator through which *all* client log calls flow, so no call site (current
  or future) can emit an unmasked value and the guarantee holds by construction rather than by per-site
  discipline. The decorator replaces every non-null `udf*` value in a log line's `meta` with
  `[redacted - N characters]` (N = the original string length) before the underlying logger sees it,
  covering leniency diagnostics, debug payloads, and any later log site uniformly. Masking preserves
  the surrounding structure so a redacted log stays diagnostically useful (which fields were present,
...
- **Dual-layer rate limiter** (R11, R12) — a read sliding window (600/60 s), an aggregate write
  ...
  `operationWriteStatus` (e.g. `device-udf-set` → 600, `alert-resolve` → 100). The opKey→limit map is
  a **committed static table** (`src/rate-limits.ts`), seeded from the observed `system/request_rate`
  contract, so the limiter has concrete limits before the first request (init cannot depend on a prior
  `requestRate()` call); a write opKey with no explicit entry **falls back to the common 100 write
  ceiling** (the observed default for every write except `device-udf-set`). `system.requestRate()`
  stays available for a consumer to reconcile against the live server budget, but the client does not
  read it at init — adaptive throttling from that endpoint is a Future Consideration. The classification
...
carrying a future server-side enum value; so response validation is lenient and widens enums to
passthrough (see the lenient-parsing Key Concept), while requests stay strict. Splitting the fix by kind —
...
**Rationale:** 57 operations on one object is unnavigable; grouping by resource matches the API's own
...
**Alternatives considered:**
- *Flat methods (current style).* Rejected: 57 same-level methods, poor discoverability, and
...
`users`, `activityLogs`, `system` — collectively covering all 53 paths (R1). Naming rule, fixed at the
`1.0.0` freeze: **plural** for collection namespaces (`sites`, `devices`, `alerts`, `jobs`, `filters`,
...
- All 53 paths / 57 operations are reachable through resource namespaces (R1, R2), each with a
...
- A response fixture carrying an **unobserved enum value** both type-checks against the codemod-widened
  response type and survives `parseLenient` without being dropped — verifying the build-time type
  widening and runtime enum widening stay aligned on the same response-enum field set (R5).
...
- The README covers install, auth, every namespace (with the namespace→endpoint map), error handling,
  logger injection, leniency, and rate limiting (R18); the package publishes as ESM `1.0.0` with
  `dist` + types (R16, R19).
...
| Real-response fixtures (R17) contain live secrets — UDFs observed to hold BitLocker recovery keys, admin usernames, credentials — that must not be committed, nor leaked to logs at runtime | High | High | At rest: a **committed, deterministic sanitization script** (`scripts/sanitize-fixtures.mjs`) redacts every `udf*` field (and other values under a fixed set of secret-bearing keys) to `null` while preserving type/nullability shape, so a maintainer capturing real data commits a scrubbed fixture; added real captures are additionally reviewed at commit time (the currently committed captures hold only benign `udf1:"value1"`/`"value2"`). Redaction is **key-based**, not content-based: we deliberately do **not** ship an automated secret *detector*/scanner, because a reliable abstract "is this value a secret" heuristic is not achievable — it false-positives on the committed OpenAPI document's prose and OAuth structural keys and false-negatives on novel secret shapes. At runtime: UDF log masking (R20) keeps UDF values out of log output. |
...
- **Extracting the shared leniency / base-resource / error primitives into a common internal
  package**: deferred until both clients are on the throwing model. `parseLenient` and `BaseResource`
  are ported from `fuze-api` as a **conscious, temporary copy** for this single-client rebuild; a
  follow-up will evaluate consolidating them so the two clients do not skew on the next fix to either
  side.
- **A streaming `paginate` (async-iterator / page-cursor) variant for very large collections**: the
  `1.0.0` surface eagerly buffers the full result set (matching 0.1.x and R3's stated intent); a
  streaming escape hatch is additive and can be introduced later without a break.
