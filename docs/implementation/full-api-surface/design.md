...
|----|-------------|------|--------|
| R1 | The client exposes **every** Datto RMM API v2 operation — all 53 paths / 75 operations across the ten namespaces `account`, `sites`, `devices`, `alerts`, `jobs`, `filters`, `audit`, `users`, `activityLogs`, `system` — covering both reads and writes. | Functional | Scope decision (full read + write) |
