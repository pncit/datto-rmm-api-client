...
| Real-response fixtures (R17) contain live secrets — UDFs observed to hold BitLocker recovery keys, admin usernames, credentials — that must not be committed, nor leaked to logs at runtime | High | High | At rest: a **committed sanitization script** redacts/synthesizes secret-bearing fields (notably `udf*`) while preserving type/nullability shape, and a **pre-commit + CI scan** over `spec/` and fixture paths **fails the build** when an unsanitized `udf*`/credential-shaped value is present — so "never commit raw sweep output" is enforced mechanically, not by remembering a manual step. At runtime: UDF log masking (R20) keeps UDF values out of log output. |
