...
| R3 | Paginated collections transparently walk `pageDetails.nextPageUrl` and return the **full** result set. | Functional | Existing pagination behavior, generalized |
...
| R5 | **Response** validation is **lenient**: unknown keys are stripped and logged, and all fields are tolerated as nullable/optional (the spec carries no reliable presence/nullability information). | Non-functional | Reality findings |
| R6 | **Request** bodies are validated **strictly** before sending. | Functional | fuze-api pattern |
...
| R15 | The OpenAPI spec is committed (`spec/openapi.json` + a previous copy for diffing), regenerated via a documented `npm run generate`; generated output is deterministic and never hand-edited. | Non-functional | fuze-api pattern |
...
burden is nullability, timestamps, UDFs, and alert context, not enums.
...
- **Spec-patch step** (R8, R15) — a committed script transforms `spec/openapi.json` into a patched
  spec *before* Orval, fixing structural defects generation cannot infer: timestamp fields
  `string → integer`, and `Alert.alertContext` replaced with a permissive open object carrying
  `@class`. Deterministic and reproducible.
- **Schema-override module** (`src/schema-overrides.ts`, R8) — where a per-field patch is clearer
  ...
  rather than 300 literal keys), a small hand-maintained module wraps the generated schema. Imports
...
- **Lenient response parsing** (`src/validation/schema-leniency.ts`, R5, R7) — `parseLenient`
  ...
  null/absent values on any field (the spec cannot be trusted for presence). Unknown keys are
  stripped and logged; array items validate independently and drop-and-log on failure.
...
- **Dual-layer rate limiter** (R11, R12) — a read sliding window (600/60 s), an aggregate write
  ...
  `operationWriteStatus` (e.g. `device-udf-set` → 600, `alert-resolve` → 100). Each write operation
  declares its operation key; the limiter enforces the tightest applicable window.
...
**Rationale:** The spec is accurate on structure and enums but *systematically wrong* on nullability
(zero modeled; pervasive in reality), timestamp types, UDF representation ergonomics, and alert
context. Verbatim generation would reject real responses in strict mode. Splitting the fix by kind —
