...
- **Schema-override module** (`src/schema-overrides.ts`, R8) — where a per-field patch is clearer
  ...
  **`pageDetails` cursor override** (R3) — `nextPageUrl`/`prevPageUrl` as nullable strings, `count`/
  `totalCount` present — that the strict `paginate` walk validates against. Imports generated schemas;
...
- **Lenient response parsing** (`src/validation/schema-leniency.ts`, R5, R7) — `parseLenient`
  ...
  **response type** is widened the same way (`EnumUnion | (string & {})`) so the compile-time contract
  ...
  exhaustive union. The `EnumUnion | (string & {})` "open-enum" idiom is TypeScript-only and has no
  JSON-Schema representation, so it cannot come from an Orval spec-level hook (a spec enum widened
  with a string collapses to plain `string`, losing the literal members). It is instead produced by a
  **committed post-generate codemod** run as the second step of `npm run generate` — exactly the
  pattern `fuze-api` establishes (`"generate": "orval && node scripts/dedupe-generated-index.mjs"`).
  The codemod walks Orval's emitted types output and rewrites every response enum field to the widened
  `EnumUnion | (string & {})` form — the same generic, field-agnostic move `parseLenient` makes at
  runtime, applied once deterministically across the generated output rather than hand-edited per
  field, so it stays reproducible under R15 (a script, not a hand-edit) and its output is re-derived
  byte-for-byte on every regeneration. Unknown keys are
...
- **`BaseResource`** (`src/client/resources/base-resource.ts`, R2, R6, R7) — validated
  ...
  The `pageDetails` cursor itself is validated by a dedicated **override** schema (not the lenient item
  path): `nextPageUrl`/`prevPageUrl` are modeled as **nullable** strings and `count`/`totalCount` as
  present integers, and the shape is enforced **strictly** — a missing or malformed `pageDetails` throws
  `DattoValidationError` instead of ending the walk early and returning a partial set, while a `null`
  `nextPageUrl` is the ordinary terminal that stops the walk cleanly. Modeling the cursor as nullable is
  required because the spec models no nullability (Current State defect #1) yet the terminal page
  legitimately returns `null` — a verbatim non-null cursor schema would throw at the end of every walk.
  Leniency governs the item payloads, never the walk-control cursor.
