...
- `src/client/resources/base-resource.ts`: `BaseResource` — the validated HTTP primitive set
  (`httpGet`/`httpPost`/`httpPut`/`httpPatch`/`httpDelete`), `validateRequest` (strict, R6),
  `validateResponse`/`validateArrayResponse` (lenient, R5/R7, with per-call aggregated drop
  reporting), the strict-cursor `paginate` walker (R3), and the ported `coerceSchema` helper.
- `src/schema-overrides/`: the hand-maintained module reconciling the generated schemas —
  ...
  `alertResponseSchema`, `ALERT_WIDENED_FIELDS`), `write-bodies.ts` (`udfWriteBodySchema`,
  `DeviceUdfInput`), `types.ts` (the reconciled `Device`/`Alert` entity types and the
  `OVERRIDE_ENTITIES` registry), `index.ts` (barrel).
...
- Any resource class (`AccountResource`, `DeviceResource`, …) or `DattoRmmClient` itself (Phase
  ...
  not the resources.
- Write-body required-field marks beyond `device-udf-set` — see §6 Decision 4 for the scoping
  rationale.
...
|------|------------|-----------|
| `src/client/resources/base-resource.ts` | Created | `BaseResource`: `httpGet`/`httpPost`/`httpPut`/`httpPatch`/`httpDelete`, `validateRequest`/`validateResponse`/`validateArrayResponse`, `paginate`, `coerceSchema` |
...
| `src/schema-overrides/write-bodies.ts` | Created | `udfWriteBodySchema`, `DeviceUdfInput` — the write-body required-field-mark pattern and its one worked example |
| `src/schema-overrides/types.ts` | Created | Reconciled `Device`/`Alert` types (the `z.infer`+open-enum-graft), `OVERRIDE_ENTITIES` registry |
| `src/schema-overrides/index.ts` | Created | Barrel |
| `tests/unit/client/base-resource.test.ts` | Created | Primitive/validate-method coverage (19 tests, nock) |
| `tests/unit/client/paginate.test.ts` | Created | `paginate` walk/cursor/drop/rate-descriptor coverage (7 tests, nock) |
...
| `tests/unit/schema-overrides/write-bodies.test.ts` | Created | `udfWriteBodySchema` (3 tests) |
...
five `http*` primitives instead of the plan's four — see §5 Deviation 1 — each attaching an
explicit `RateDescriptor` to the axios request config so every request a resource makes is rate-
limited: `httpGet` tags `{kind:'read'}`; `httpPost`/`httpPut`/`httpPatch` each accept a typed
`WriteOpKey` and tag `{kind:'write', opKey}`, via a shared private `sendWrite` dispatcher that
branches on a 3-arg (bodiless) vs. 5-arg (body-carrying) argument tail so the same primitive name
serves both a bodiless write (`POST /alert/{uid}/resolve`) and a body-carrying one (`POST
/device/{uid}/udf`) without a second primitive name; `httpDelete` sends no body and validates no
response (every real Datto `DELETE` operation's documented responses carry no content — verified
against the committed spec). `validateRequest` is the strict R6 path (`schema.safeParse`, throwing
`DattoValidationError('request')` on the first failure — the plan's pinned 2-arg construction).
`validateResponse` runs `parseLenient` (always, since `logger` is a required constructor param —
R5) and re-asserts the narrower declared type over `parseLenient`'s actual `Lenient<T>` return (see
that method's doc for why this is a documented, intentional narrowing, not an invented guarantee).
`validateArrayResponse` validates each item independently and emits **one** aggregated `warn`
summary per call for every drop (R7) — never one line per dropped row — capping the reported
per-item errors at 5 (`MAX_REPORTED_DROP_ERRORS`) so a systematic drift (one mistyped field
dropping an entire 848-item page) still produces a single, bounded `warn`. `paginate` walks
`pageDetails.nextPageUrl`, validating each page's cursor strictly against `pageDetailsSchema`
(throwing `DattoValidationError('response')` on a missing/malformed cursor rather than truncating —
R3) and each page's named array leniently via `validateArrayResponse`; it attaches its own explicit
`{kind:'read'}` descriptor per page since it talks to the shared axios instance directly rather
than through `httpGet` (it reads a two-part envelope, not a single schema-validated value).
...
`Alert.responseActions[number].actionType`) depths via a scratch compile check (§7). `write-
bodies.ts` establishes the required-field-mark pattern with one worked, tested example:
`udfWriteBodySchema` (`.refine`s the generated `setUdfFieldsBody` to reject an all-`undefined`
body) and `DeviceUdfInput`.

---

## 5. Deviations From Plan (If Any)

1. **Five HTTP primitives, not four: `httpPut` added.** The plan (mirroring `fuze-api`) names
   exactly `httpGet`/`httpPost`/`httpPatch`/`httpDelete`. While building this phase I read every
   non-`GET` `(method, path)` pair in the committed `spec/openapi.json` (the plan's own source of
   truth, fetched in Phase 2) and found Datto's real v2 API uses **no `PATCH` operations at all**
   (0 of 18 non-`GET` operations) and instead uses **`PUT`** for five operations this project's own
   `WriteOpKey` table (Phase 5) requires: `moveDevice` (`device-move`), `createQuickJob`
   (`device-job-create`), `create` site (`site-create`), `createSiteVariable` (half of
   `site-variable-set`), and `createAccountVariable` (half of `account-variable-set`). Without a
   validated `httpPut` primitive, Phase 7/8 could not implement these required (R1) operations
   without either bypassing `BaseResource` entirely (a resource calling `this.axios.put` directly —
   defeating the single-validated-primitive architecture this class exists to provide, and the
   design's explicit "every request... is rate-limited, retried, and error-mapped by that one
   shared stack") or mis-sending them as `POST`/`PATCH` against a server that documented `PUT` for
   that path. I added `httpPut`, mirroring `httpPost`'s exact bodied/bodiless overload shape, as
   the minimal, necessary fix — this is implicit-intent handling, not scope creep: the plan's own
   discrete instruction (build the validated primitive set) is incomplete without it, since two of
   Phase 7's own named deliverables (`DeviceResource.move`, `DeviceResource.createJob`) cannot be
   correctly implemented against the real API otherwise. I kept `httpPatch` (currently unused by
   any real Datto write) rather than dropping it: it costs nothing to keep, matches the plan's
   pinned name and the `fuze-api`-parity goal (Decision 1), and a future spec revision may add a
   real `PATCH` operation.
...
2. **`validateResponse`/`validateArrayResponse` re-assert the narrower declared type `T` over
   `parseLenient`'s actual `Lenient<T>` return.** Since `BaseResource` always supplies a logger,
   `parseLenient` always takes its `Lenient<T>`-returning overload (every named field additionally
   admits `null`) — but every resource method (Phase 7/8) is meant to return the clean, documented
   `Device`/`Alert`/etc. types, not a `Lenient<Device>` doubly-nullable variant. `Lenient<T>` exists
   (Phase 4) as an internal type-testing utility pinned by `tests/generated/lenient-type-pin.ts`,
   not as the shape `BaseResource`'s own public contract should expose — re-asserting `T` (via `as
   T`, documented in the method's own doc comment) is the same kind of "runtime validates
   correctly; the TS type is an intentional, documented approximation" pattern `coerceSchema`
   already establishes elsewhere in this same file.
...
3. **`coerceSchema` is needed by, but not literally invoked in, this phase.** The plan's Phase 7
   ...
   illustrative, not mandates.

4. **Write-body required-field marking scoped to `device-udf-set` only, not "every write body."**
   Phase 6 Step 3's prose ("wrap each generated write-body schema... in this one place") reads as
   comprehensive, but I scoped this phase's actual deliverable to the module, its pattern, and one
   fully-worked, tested example (`udfWriteBodySchema`) — the one the plan's own Phase 6 Tests
   section names ("a write-body override rejects an empty body," singular) and the one Phase 7's
   own example code depends on by name. Hand-verifying "genuinely required" fields for the other
   ~9 body-carrying writes (`createQuickJob`, `setWarrantyData`, site/variable creates, proxy
   settings) has no resource call site to validate against yet — those resources are explicitly
   Phase 7/8's named deliverables ("implement operations as thin methods that select the... correct
   rate-limit `opKey`"), and speculatively marking their required fields now, with nothing
   exercising them, risks guessing wrong against the "endpoint docs" with no test to catch it. The
   module and its pattern (this phase's actual architectural deliverable) are complete; the
   remaining bodies are added to this same file as their owning resource is implemented.

---

## 7. Tests

- `tests/unit/client/base-resource.test.ts` (19 tests, nock): `httpGet` tags `{kind:'read'}` and
  strips an unknown response key; `httpGet` forwards query params; `httpPost` bodiless sends no
  body and tags `{kind:'write', opKey}`; `httpPost` body-carrying validates and sends the body;
  `httpPost` throws `DattoValidationError('request')` **without sending** when the body fails
  validation (`scope.isDone()` asserted `false`); `httpPut` bodiless/body-carrying (mirroring
  `httpPost`); `httpPatch` body-carrying; `httpDelete` sends no body/response validation and tags
  the descriptor; `validateRequest` throws on an unknown key and on a missing required field, and
...
- `tests/unit/client/paginate.test.ts` (7 tests, nock): a two-page walk concatenates items and
  stops on `nextPageUrl: null`; a page terminating with `nextPageUrl: ""` (the real Datto terminal
  form) also stops the walk; a missing `pageDetails` throws `DattoValidationError` rather than
  truncating; a non-string `nextPageUrl` throws the same; a lenient (bad) item on page 2 is dropped
  without aborting the walk; the walk consumes the read rate-limit window exactly once per page
  fetched (asserted via the captured `RateDescriptor` array); the initial `params` argument is sent
...
- `tests/unit/schema-overrides/write-bodies.test.ts` (3 tests): `udfWriteBodySchema` accepts a body
  with one udf set, rejects an empty body, and rejects an unknown key.
...
- **Write-body required-field marks beyond `device-udf-set`** (`device-job-create`,
  `device-warranty-set`, `site-create`, `site-variable-set`, `account-variable-set`,
  `device-proxy-set`) are not yet added to `write-bodies.ts` — deliberately deferred to Phase 7/8
  as each owning resource is implemented (§6 Decision 4). One finding worth flagging now:
  `setWarrantyDataBody`'s generated field (`warrantyDate: z.string().optional()`) does not allow
  `null`, even though the endpoint's own doc says "the warranty date can also be set to null" — the
  Phase 7/8 override for this body will need `.nullable()`, not just a required-field mark.
...
- `npm test` — 293 tests passing across 25 files (50 new in this phase; every pre-existing test
  from Phases 1–5 still green, confirming coexistence).
...
- `npx prettier --write` — applied to every new file for formatting consistency, followed by a
  full re-run of lint/typecheck/test/build to confirm no behavioral change.
...
I assert that:
- Only Phase 6 has been implemented.
- No unnecessary scope expansion occurred — the one substantive deviation from the plan's literal
  primitive list (`httpPut`) is a documented, necessary addition grounded in the committed spec,
  not a preference, and Phase 5's rate-limit table (a genuine, related finding) was left untouched
