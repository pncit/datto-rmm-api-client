...
>(
  ...
  while (nextUrl) {
    ...
    if (this.validationMode === "off") {
      ...
      if (!parsed.success) {
        // Compute the failing path ONCE; reuse it for both the log line and the ProblemError.detail so all
        // three validation-error sites share the concise, path-named convention and detail is never a raw blob.
        const envelopePath = parsed.error.issues[0]?.path?.join(".") || "(root)";
        // Single-line, path-named (like every other log in the feature); full error stays in raw, not the log line.
        logger.error(`Malformed devices page envelope at ${nextUrl} (path: ${envelopePath})`);
        return {
          ok: false,
          error: {
            type: VALIDATION_ERROR_TYPE,               // shared constant, not a hand-written literal
            title: "Malformed devices page envelope",  // short stable title
            status: VALIDATION_ERROR_STATUS,
            detail: `Malformed devices page envelope (path: ${envelopePath})`, // concise + path-named, matching toProblemError
            raw: parsed.error,                          // full ZodError lives ONLY here, not duplicated into detail
          },
        };
      }
      page = parsed.data;
...
- **Strict, malformed envelope (R5) + log:** response where `devices` is not an array (e.g. `"devices": "nope"`) → `{ ok: false, error: { type: "validation-error", title: "Malformed devices page envelope" } }`, `error.detail` is the **concise, path-named** one-liner (starts with `"Malformed devices page envelope (path:"` and contains the failing path segment, e.g. `devices`) and is **not** the multi-line `ZodError.message` blob (assert it does not contain a newline), and `logger.error` called once (envelope failure is observable through the configured logger, not silent). This mirrors the per-device `detail` assertion so all `validation-error` sites are pinned to one `detail` convention.
...
- **Strict, mid-walk envelope hard-fail discards earlier pages (R5, design Success Criteria L184 + Verification L196):** page1 `[valid1, divergent1]` with `nextPageUrl` → page2, page2 with a **malformed envelope** (e.g. `"devices": "nope"`) → `result.ok === false`, `error.type === "validation-error"`, `error.title === "Malformed devices page envelope"`, **no** partial `value` (the walk returns the `{ ok: false }` mid-loop, so `valid1` and `divergent1`'s warning accumulated from page1 are **discarded**, not partially returned), and `logger.error` called for the envelope failure. This is the one path with distinct *discard* semantics — behaviorally different from a first-page envelope failure — and pins that a later-page envelope failure throws away the accumulator rather than returning it, exactly as pagination cannot continue past an unreadable page. (Note: page1's divergent device also fires a per-device `logger.error` before page2 is fetched; assert `logger.error` was called for the envelope message specifically, e.g. via a message match on `"Malformed devices page envelope"`, rather than a strict call-count of 1.)
