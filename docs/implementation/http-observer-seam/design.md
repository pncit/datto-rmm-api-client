...
- Observing non-HTTP failures. Response schema validation (`DattoValidationError`) and pagination cursor/guard failures are not HTTP-attempt errors and do not fire `onError` (see Decision 4).
