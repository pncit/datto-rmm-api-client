...
| R8 | `onError`'s error field is typed `unknown` and delivered as the raw request error exactly as the transport produced it — the seam makes no promise to map it (a `throw` guarantees nothing about an error's shape). No callback field's type references an axios type. | Non-functional | Non-Goal: axios stays out of the contract |
...
- when the attempt returns a non-2xx or no response, `onError` fires with the request fields, the response fields (when a response was received), the raw request error (typed `unknown`, handed off as-is — see Decision 4), and the elapsed wire time.
...
interface DattoHttpRequestEvent {
  method: string;
  url: string;                // the absolute resolved request URL (baseURL + path) exactly as dispatched — for both a resource request and the grant, composed via the instance's own `getUri` (axios's `buildFullPath`/`combineURLs`), never a bare relative path or a manual concatenation (Decision 5, R3/R4)
...
interface DattoHttpErrorEvent {
  ...
  requestHeaders: DattoHttpHeaders;
  requestBody: unknown;
  error: unknown;                    // the raw request error, handed off as-is (Decision 4)
...
#### Decision 4: `onError.error` is the raw request error (`unknown`), and the seam is HTTP-only

**Decision:** Type `onError.error` as `unknown` and hand off the raw request error exactly as the transport produced it — whatever its shape — without re-mapping it to a `DattoApiError`. A `throw` makes no guarantee about an error's shape, so the seam makes none either: it hands off the request error, whatever that is, regardless of what the SDK ultimately returns to the caller. The response fields the event still carries (`statusCode`/`responseHeaders`/`responseBody`, present when a response was received) give the consumer structured context without the seam pretending a shape it cannot guarantee. Terminal-event selection follows three rules:
...
**Rationale:** A `throw` carries no shape guarantee — the transport may reject with an `AxiosError`, a `DattoApiError` (the grant path constructs one), or any other value — so typing `onError.error` as anything narrower than `unknown` would over-claim. The observer's job is to hand off *the request error the transport actually produced*, faithfully, not a re-derived stand-in: re-mapping in the observer risks the observed error diverging from what the client actually throws to the caller (e.g. a terminal 429, which the client surfaces as `buildRateLimitError` carrying `retryAfterMs`, could not be reproduced by a generic re-map that lacks the retry context). Handing off the raw error keeps the seam honest and cheap, and the consumer still gets structured context from the event's response fields. The non-dispatched gate (rule 2) is honest because both excluded paths throw from a request interceptor that runs *before* the observer interceptor (registered **first** → runs **last**, per Decision 5), so neither attempt reached the observer's dispatch-point capture nor holds a stash; firing `onError` at the top of `handleResponseError` — or guarding only the rate-limiter case — would double-report a grant failure already surfaced on the grant client's own `onError`, breaking the "one observed attempt ⇒ one terminal event" invariant. Consistent with Decision 5, the grant instrumentation fires `onResponse` off the resolved 2xx **before** `safeParse` runs, so a post-parse throw cannot re-enter a terminal event. Delivering `onError` on retried non-terminal attempts (429/5xx/401) simply hands off the raw `AxiosError` for that attempt even though the client swallows it and retries — no `DattoApiError` construction is required on those branches.

**Alternatives considered:**
- `error: DattoApiError` (map every observer error to the client's error type): rejected per human ruling — a `throw` guarantees nothing about an error's shape, and re-deriving a `DattoApiError` in the observer risks diverging from what the client actually throws (the terminal-429 `buildRateLimitError` case the observer cannot reproduce without the retry context), making the audit artifact dishonest. Handing off the raw error keeps the seam faithful.
- `error: DattoApiError | DattoValidationError`: rejected — narrows harder than `DattoApiError` while `DattoValidationError` is itself a post-exchange, non-HTTP failure outside this seam's scope; including it would imply the seam fires on validation failures, which it does not.
...
The captured `url` is the **absolute resolved** request URL — `baseURL` concatenated with the path, including the serialized query string — not the bare relative path axios holds in `requestConfig.url`. The shared instance composes it via `instance.getUri(requestConfig)` at the dispatch point (`engineer-r1-f1`/`f2`): `getUri` runs axios's own `buildFullPath`/`combineURLs` and appends the serialized `requestConfig.params`, so the observed URL matches exactly what axios dispatches — including any query string, correct slash-joining, and axios's own absolute-`url` handling — rather than a manual `` `${baseURL}${url}` `` concatenation, which drops `params` (not yet serialized when the request interceptor runs) and can diverge from axios's own joining rules. The grant path composes it the same way, via `this.grantClient.getUri({ url: GRANT_PATH })`: because the grant client is built with `baseURL: apiUrl`, `getUri` runs the same `buildFullPath`/`combineURLs` join, so the observed grant URL matches exactly what axios dispatches even when `apiUrl` carries a trailing slash — a manual `` `${apiUrl}${GRANT_PATH}` `` concatenation would double the slash (`https://host//auth/oauth/token`) and diverge from the dispatched URL. The grant attaches no query params, so no `params` serialization is involved there, but routing through `getUri` keeps the two instrumentation sites identical in how they resolve the absolute URL. A bare relative path, or one missing its query string, would be an incomplete audit artifact for "every outbound HTTP exchange" — the consumer's pipeline must be able to record which host and which query each exchange hit, so the fully resolved URL is pinned rather than left transport-relative or query-stripped.
...
- Every event's `url` is the **absolute resolved** URL, composed via the instance's own `getUri` (axios's `buildFullPath`/`combineURLs`) for both a resource request and the grant, never a bare relative path or a manual concatenation — so a trailing-slash `apiUrl` resolves the grant URL with a single slash, matching what axios dispatches.
...
- `onError.error` is the raw request error (typed `unknown`) handed off unchanged, with response fields (`statusCode`/`responseHeaders`/`responseBody`) present when a response was received and absent for transport failures.
...
| The raw `onError.error` for a retried attempt is the transient `AxiosError`, which a consumer might mistake for a terminal failure | Low | Low | Each attempt is one observed exchange by design (R2); the consumer distinguishes terminal from retried by the subsequent `onResponse`/`onError` for the logical operation. Handing off the raw error avoids constructing a `DattoApiError` per retry entirely. |
