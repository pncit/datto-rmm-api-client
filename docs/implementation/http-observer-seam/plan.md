...
2. **Register the observer request interceptor FIRST**: Inside `createHttpClient`, register the observer's request interceptor **before** the rate-limit interceptor so that under axios LIFO it runs **last** — after rate-limit acquisition and after the Bearer interceptor `AuthManager.attachTo` adds later. In it, build the capture through the **shared `captureRequest` assembler** (never inline) with the **absolute resolved** `url` — composed via `instance.getUri(requestConfig)` (architect-r1-f2, engineer-r1-f1/f2), which runs axios's own `buildFullPath`/`combineURLs` and appends the serialized `requestConfig.params`, so the observed URL — including its query string — matches exactly what axios dispatches, rather than a manual `` `${baseURL}${url}` `` concatenation that would omit not-yet-serialized `params` and could diverge from axios's own slash-joining/absolute-`url` handling — passing `requestConfig.method`, `requestConfig.headers`, and `requestConfig.data`; stash it onto `requestConfig.__dattoObserverCapture` (unconditionally overwritten every pass), then fire `onRequest` from the stash.
   - Files: `src/http/http-client.ts`
...
if (config.httpObserver) {
  instance.interceptors.request.use((requestConfig) => {
    ...
    const capture = captureRequest({
      ...
      url: instance.getUri(requestConfig), // absolute resolved URL, incl. serialized params — matches what axios dispatches
...
- `tests/unit/http/http-client.test.ts` (extend), using `nock`:
  ...
  - A GET carrying `params` is observed with the **serialized query string included** in `url` (`instance.getUri`), not silently dropped (engineer-r1-f1).
  - A `paginate`-shaped first-page request — a GET issued with its cursor/filter state via `params:` (mirroring `BaseResource.paginate`'s first request, which only inlines `pathname + search` into `url` for *subsequent* pages) — is observed with that `params`-carried query string present in `url` (engineer-r1-f1).
