...
describe("AuditResource", () => {
  ...
  it("getPrinter() hits GET /api/v2/audit/printer/{deviceUid}", async () => {
    ...
    const result = await resource.getPrinter("dev-1");
  ...
  it("getEsxiHost() hits GET /api/v2/audit/esxihost/{deviceUid}", async () => {
    const scope = nock(BASE_URL)
    ...
    const result = await resource.getEsxiHost("dev-2");
  ...
  it("getDevice() hits GET /api/v2/audit/device/{deviceUid}", async () => {
    const scope = nock(BASE_URL)
      .get("/api/v2/audit/device/dev-3")
      .reply(200, { systemInfo: { manufacturer: "HP", username: "user1" } });
    const { resource } = makeResource();

    const result = await resource.getDevice("dev-3");
  ...
  it("getDeviceSoftware() paginates GET /api/v2/audit/device/{deviceUid}/software", async () => {
    const scope = nock(BASE_URL)
    ...
    const result = await resource.getDeviceSoftware("dev-3");

    expect(result).toEqual([{ name: "Chrome", version: "120.0" }]);
    expect(scope.isDone()).toBe(true);
  });

  it("getDeviceByMacAddress() hits the bare-array endpoint and drops a malformed item (R7)", async () => {
    const scope = nock(BASE_URL)
    ...
    const result = await resource.getDeviceByMacAddress("AA-BB-CC-DD-EE-FF");
